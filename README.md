...
## 4. 현재 진행 현황 (Phase 3: 한국어 키워드 모델 전환)
현재 영어 기반("yes", "no") 모델을 한국어 교육 환경 특화(타겟 단어: **"퀴즈"**, **"이해하셨나요"**) 모델로 전환하는 데이터 수집 및 학습 파이프라인 작업을 진행 중입니다.
- **데이터 증강 기능 확보**: `for_dataset/` 폴더 내에 윈도우 스피커 루프백 직접 녹음 스크립트(`web_audio_recorder.py`)와, 노이즈 및 대역통과 필터(Bandpass Filter)를 활용한 실관경 모사 데이터 증강 스크립트(`augment_audio.py`)가 추가되어 있습니다.
