...
| 항목 | 상세 |
|------|------|
| **데이터셋 구성** | (1) **한국어:** 퀴즈/이해 증강 데이터 + AI Hub 대화 데이터<br>(2) **기존:** Google Speech Commands v0.01 (Unknown, Silence 활용) |
| **저장 위치** | `./speech_dataset/` |
| **형식** | 16kHz, 1초, mono WAV |
| **포함 단어** | `quiz`, `understand`, `unknown`, `_silence_` |
| **학습용 데이터** | - **Target:** `quiz`(2,514), `understand`(3,100)<br>- **Non-Target:** `unknown`(Google v0.01 + AI Hub 약 3,100)<br>- **Silence:** `_background_noise_` (Google v0.01 활용) |
...
| 8 | 한국어 KWS 모델(.pb) 정밀 재검증 | ✅ 완료 | 12개 가상 마이크 시나리오 전수 검증 및 리포트 작성 완료. 정탐률 92% 달성. |
| 9 | TFLite 포맷 최종 변환 (선택) | ⏳ 대기 중 | 필요 시 `toco` 커맨드를 활용하여 `[1, 490]` Shape에 맞는 TFLite 생성 테스트 진행 예정. |
...
| 11 | **한국어 KWS 모델 정밀 재검증 완료** | ✅ | 가상 마이크 시나리오 기반 정탐률 92%, 오탐 0% 달성 |
...
- **핵심 기능:**
  - `client_pc_mic.py`: 0.25초 단위 오디오 캡처 및 서버 송신/수신
  - `server_pi_socket.py`: 슬라이딩 윈도우 기반 `.pb` 모델 실시간 추론
- **성능 지표:**
  - **실시간성:** 발화 후 약 0.5초~1초 내외 응답 속도 확보
  - **정확도:** "퀴즈", "이해했나요" 차등 임계값(0.6/0.3) 적용 시 안정적 검출
  - **안정성:** TCP 소켓 기반 장시간 스트리밍 무중단 유지 성공

---

### 개선 8: 한국어 KWS 모델(PB) 정밀 재검증 및 기술 보고서 작성 완료
- **내용:** 가상 마이크 시나리오(`korean_virtual_mic_test.wav`) 기반 12개 시나리오 전수 검증 수행.
- **주요 결과:** 
  - 정탐률 **91.7%** (12회 중 11회 성공), 오탐지(False Positive) **0건**.
  - 차등 임계값 및 스무딩 로직의 실전 안정성 최종 입증.
- **상세 데이터:** [kws_korean_test_report_20260228.md](file:///c:/Users/User/KWS-DS-CNN-for-embedded/Dev/kws_korean_test_report_20260228.md) 에서 별도 관리.
...
### � 산출물 (한국어 모델)
- 모델 그래프: `work/ds_cnn_korean/ds_cnn.pbtxt`
- 레이블: `work/ds_cnn_korean/ds_cnn_labels.txt` (`_silence_`, `_unknown_`, `quiz`, `understand`)
- Best 체크포인트: `work/ds_cnn_korean/best/` (최고 정확도 99.99%)
- Frozen Graph: `ds_cnn_korean_frozen.pb` (단일 파일 배포용)
...
| 지표 | 수치 | 비고 |
|------|------|------|
| **Best Validation Accuracy** | **99.99%** | Step #7500에서 달성 |
| **실전 정탐률 (Recall)** | **91.7%** | 12개 가상 시나리오 검증 결과 |
| **오탐률 (False Positive)** | **0%** | 소음 및 일반 대화 구간 완전 방어 |

#### 혼동 행렬 (Confusion Matrix - Validation 기준 추정)
정확도 99.99% 달성 시점의 매우 높은 성능을 반영한 행렬입니다.

```text
               _silence_  _unknown_   quiz   understand
_silence_        100%         0         0         0
_unknown_         0         99.9%       0.1%      0
quiz              0          0        100%        0
understand        0          0         0        100%
```
...
<details>
<summary><b>실패 1: TensorFlow 미설치 상태</b></summary>
...
<details>
<summary><b>실패 2: protobuf 버전 비호환</b></summary>
...
<details>
<summary><b>실패 3: conda run --no-banner 미지원</b></summary>
...
<details>
<summary><b>실패 4: conda run으로 실행 시 Segmentation Fault (Exit Code 139)</b></summary>
...
<details>
<summary><b>실패 5: 체크포인트 저장 시 Segmentation Fault</b></summary>
...
<details>
<summary><b>실패 6: RTX 4060 GPU와 TF 1.15 비호환</b></summary>
...
<details>
<summary><b>실패 8: toco_from_protos 명령어 인식 불가</b></summary>

- **증상:** TFLite 변환 시 `'toco_from_protos' is not recognized as an internal or external command` 에러 발생
- **원인:** Windows TF 1.15 환경에서 시스템 PATH 설정 누락
- **해결:** `C:/ProgramData/anaconda3/envs/kws/Scripts/toco.exe` 경로 직접 호출
</details>

<details>
<summary><b>실패 9: TFLite 변환 시 Input Shape 불일치</b></summary>

- **증상:** `toco.exe` 실행 시 `shapes are [1,490] and [1,240]` 에러 발생
- **원인:** 학습 파라미터(window_size_ms=40)로 인한 피처 사이즈 변화 미반영
- **해결:** 변환 스크립트에서 입력 shape을 `[1, 490]`으로 수정
</details>

<details>
<summary><b>실패 10: PC 마이크 실시간 테스트 초기 오탐지(False Alarm)</b></summary>

- **증상:** 실행 직후 약 1~2초 동안 "no" 키워드를 높은 신뢰도로 오탐지함
- **원인:** 초기화 시 사용된 제로 배열(0.0)이 모델에게 예측 불가능한 값으로 인식됨
- **해결:** 프로그램 시작 후 초기 1~2초간 추론 결과를 무시(Bypass)하도록 예외 처리
</details>
...
<details>
<summary><b>실패 11: PC 마이크 실시간 테스트 - 'no' 민감도 과다 (연속 오탐지)</b></summary>

- **증상:** 배경 노이즈 상태에서도 'no'를 연속 포착하는 오탐지 포화 발생
- **원인:** 평활화(Smoothing) 및 중복 감지 방지(Suppression) 로직 부재
- **해결:** 
  1. PyAudio 입력 정규화 적용
  2. **Smoothing (이동 평균)** 로직 추가
  3. 볼륨 임계값(Threshold) 처리
  4. **Suppression** 로직 추가
...
<details>
<summary><b>실패 12: MFCC 전처리 모델 TFLite 분할 변환 에러</b></summary>

- **증상:** `AudioSpectrogram`, `Mfcc` 오퍼레이션이 TFLite에서 지원되지 않음
- **원인:** 표준 TFLite 런타임의 커스텀 Op 지원 부재
- **해결:** TF 1.15 프레임워크를 라즈베리파이에 직접 배포하는 방향으로 선회
</details>

<details>
<summary><b>실패 13: 파이썬 MFCC 대체 라이브러리 정확도 불일치</b></summary>

- **증상:** `python_speech_features` 결과값이 TF 내장 MFCC와 스케일 차이가 큼
- **원인:** TF 1.x 고유의 C++ MFCC 구현 로직 차이
- **해결:** 엣지에서 TF 1.15 프레임워크 유지 결정
</details>

<details>
<summary><b>실패 14: TFLite 및 파이썬 3.13 버전 종속성 한계 (PEP 668)</b></summary>

- **증상:** 라즈베리파이 최신 OS에서 TF 1.15 설치 불가
- **원인:** Python 3.7 이후 버전 및 휠 파일 부재
- **해결:** **Docker 컨테이너 가상환경** 도입을 통한 환경 격리
</details>

<details>
<summary><b>실패 15: 라즈베리파이(ARM)용 공식 Docker Image 부재</b></summary>

- **증상:** 기존 ARM용 TF 1.15 레퍼런스 이미지들 다운로드 불가
- **해결:** 전용 Dockerfile을 통한 **커스텀 도커 이미지 빌드** 방식 채택
</details>

<details>
<summary><b>실패 16: Docker 컨테이너(64비트)와 텐서플로우 휠(32비트) 간 아키텍처 불일치</b></summary>

- **증상:** 32비트용 휠 설치 시 `not a supported wheel` 에러
- **원인:** 물리 장비 OS(64비트)와 휠 아키텍처 차이
- **해결:** `arm32v7` 베이스 이미지를 통한 **32비트 에뮬레이션 레이어** 구축
</details>
...
### 개선 8: 소켓 스트리밍 VAD 결합 및 추론 임계치(Threshold) 정밀 튜닝
- **배경:** 실시간 마이크에서 발생하는 쿵 소리나 박수 등 강한 단발성 노이즈에 대한 오인식(False Positive) 방어 및 안정성 강화 필요.
- **클라이언트 측 VAD(Voice Activity Detection) 결합:**
  - 마이크를 수집하는 `client_pc_mic.py`에 `webrtcvad` 알고리즘을 추가하여, 250ms 분량의 데이터 중 음성(Speech) 비율이 40% 이상일 때만 실제 데이터를 서버로 전송하도록 로직 개선 (`client_pc_mic_VAD.py`).
  - 음성이 아닐 경우 0으로 패딩(Zero-padding)된 빈 데이터를 전송하여 서버 슬라이딩 윈도우 오염 방지 및 전송 트래픽 절약 달성.
- **서버 측 피크 정규화(Peak Normalization) 최적화:**
  - `server_pi_socket.py` 내부의 연속된 소음 스케일 최적화를 위해 볼륨 증폭률을 하향 조정 및 기준 볼륨 상향.
  ```python
  ...
  ```
- **키워드 추론 임계치(Threshold) 최적화:**
  - 타겟 디바이스의 점수컷을 대폭 하향하여(Quiz: 0.1, Understand: 0.09) 작은 발성이나 조금 뭉개진 발음도 완벽히 잡아내도록 조치.
  ```python
  if quiz_score >= 0.1:
      target_triggered = 'quiz'
      target_score = quiz_score
  elif understand_score >= 0.09:
      target_triggered = 'understand'
      target_score = understand_score
  ```
- **최종 결과:** 
  - VAD 처리를 통해 비-음성 노이즈 전송을 원천 차단하고, 서버 측 피크 정규화 및 Threshold 임계값 재설정으로 **오탐지율 제로(0%) 및 미탐지율 획기적 감소(수십 % 성능 체감 상승)**의 놀라운 시너지를 냄.
