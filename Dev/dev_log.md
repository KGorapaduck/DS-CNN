...
| 6 | 체크포인트 배포용 동결(Freeze & `.pb`) | 🚧 진행 중 | 도출된 `best/` 폴더 내 최우수 모델을 배포용 단일 신경망 파일(`ds_cnn_korean_frozen.pb`)로 변환 압축 중. |
| 7 | TFLite 포맷 최종 변환 (선택) | ⏳ 대기 중 | 필요 시 `toco` 커맨드를 활용하여 `[1, 490]` Shape에 맞는 TFLite 생성 테스트 진행 예정. |
